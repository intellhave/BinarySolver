...
def BinarySolver(reconstr_func, decoder_params, x_batch, batch_size,latent_dim,currentH,currentB,rho, maxIter, sigma=100):
    ...
    def func(x):    
        return reconstr_func(x) + sigma*np.sum( (x-currentH)**2 ) 
    ...
    def fx(x): # Fix v, solve for x                
        return func(x) + rho*(n-np.dot(vt,x))**2      
